        ...
        if ANDROID and self.text:
            try:
                ...
                if texture is not None:
                    ...
                    # часть, где обычно находится курсор. Обрезаем саму
                    # текстуру, а не сдвигаем полный Rectangle за границы
                    # поля — иначе длинное название рисуется поверх экрана.
                    shown_w = min(tex_w, available_width)
                    left_u = (
                        max(0.0, 1.0 - shown_w / float(tex_w))
                        if tex_w
                        else 0.0
                    )
...
class Database:
    ...
    def get_product_list(
        ...
        sort_mode = str(sort_mode or "added").strip()
        ...
        # Цветовые группы никогда не смешиваются друг с другом. Выбранная
        # пользователем сортировка применяется только внутри каждой группы.
        status_group_sql = (
            "CASE "
            "WHEN a.next_exp IS NULL THEN 4 "
            "WHEN a.next_exp < ? THEN 0 "
            "WHEN a.next_exp = ? THEN 1 "
            "WHEN a.next_exp = ? THEN 2 "
            "ELSE 3 END ASC"
        )
        tomorrow_value = (
            date.today() + timedelta(days=1)
...
class HomeScreen(BaseScreen):
    ...
    def set_sort(
        ...
        self.sort_mode = str(mode or "added")
        self.refresh(page=0)
...
class MainApp(App):
    ...
    def on_resume(self):
        ...
        Clock.schedule_once(self._refresh_visible_home, 0.38)

    def _refresh_visible_home(self, *_):
        manager = getattr(self, "sm", None)
        if manager is None or manager.current != "home":
            return
        home = manager.get_screen("home")
        home.refresh(page=home.current_page)
    ...
    def open_home(self):
        ...
        previous_screen = self.sm.current
        ...
        # Android hides the IME asynchronously. If the list is rebuilt while
        # the window still has the keyboard-sized viewport, part of the
        # ScrollView may stay unlaid-out until the app is backgrounded. Rebuild
        # once more after that animation has completed.
        if previous_screen == "add":
            Clock.schedule_once(
                self._refresh_visible_home,
                0.38,
            )

    def dismiss_keyboard(self):
        def release_android_keyboard(*_):
            try:
        ...
        try:
            for screen in getattr(self.sm, "screens", []):
                for widget in screen.walk(restrict=True):
                    if isinstance(widget, TextInput):
                        widget.focus = False
            release_android_keyboard()
            Clock.schedule_once(release_android_keyboard, 0.12)
            Clock.schedule_once(release_android_keyboard, 0.32)
        except Exception as exc:
    ...
    def _on_activity_result(
        ...
        if (
            ...
            self.handle_product_name_result(
                result_code,
                intent,
            )
            return
    ...
    @mainthread
    def handle_product_name_result(
        self,
        result_code,
        intent,
    ):
        """Apply native ML Kit OCR output on Kivy's rendering thread."""
        screen = self.pending_ocr_screen
        self.pending_ocr_screen = None

        if result_code != -1 or intent is None or screen is None:
            return
